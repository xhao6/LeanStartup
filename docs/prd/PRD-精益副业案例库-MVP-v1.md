...
**核心页面**：
- ✅ 首页（今日精选 Top 3，卡片列表，支持下拉刷新）
- ✅ 案例详情页（五维评分可视化、操作步骤 Checklist、工具/避坑指南、风险标签）
- ✅ 历史榜单（按月份分组，支持分页加载）
- ✅ 往期榜单详情（展示当日 Top 3 案例）
- ✅ 个人中心（登录/退出、已阅榜单统计、收藏入口、订阅管理、隐私政策、用户协议、关于）

**用户系统**：
- ✅ 微信登录（wx.getUserProfile 授权，昵称/头像持久化）
- ✅ 用户资料云端存储（users 集合）
- ✅ 已阅榜单追踪（viewedRankingDates，云端幂等记录）
- ✅ 昵称编辑（弹窗编辑，实时同步）

**收藏功能**：
- ✅ 本地优先收藏架构（localStorage + 云端同步）
- ✅ 收藏/取消收藏（toggleCollection 云函数）
- ✅ 操作步骤进度追踪（debounce 保存，防抖处理）
- ✅ 收藏列表页（显示标题、评分、完成进度）
- ✅ 个人中心显示收藏数量
...
- ✅ hover-class 替代 :active（消除滚动时点击态残留）
- ✅ 响应区域优化（最小 44px 触控目标）
- ✅ TabBar 3 标签（今日/榜单/我的）
- ✅ 返回键拦截（TabBar 页面跳转首页）
- ✅ 分享功能（转发给朋友）

**后端云函数**：
- ✅ getCaseDetail、getDailyPick、syncCaseDataPublic
- ✅ getUserCollections、toggleCollection
- ✅ subscribeMessage（订阅消息发送）
- ✅ generateDailyPick（定时任务）
- ✅ userFunctions（登录/资料/已阅榜单/收藏查询）
- ✅ trackEvent（埋点）

**内容管道**：
- ✅ article-downloader（Chrome CDP 文章下载）
- ✅ article-processor（LLM 评分 + 结构化提取 + 脱敏）
- ✅ sync-to-db（支持增量同步）
- ✅ 22 篇精选案例入库（ID: 100001-100022）
