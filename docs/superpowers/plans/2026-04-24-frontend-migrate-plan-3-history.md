# Plan 3: 历史榜单页 (history list, history detail)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task.

**Goal:** 将 LeanSkill 的历史榜单列表页和详情页迁移到 LeanStartup。

**Architecture:** 从 LeanSkill 复制 `pages/history/index.vue` 到 LeanStartup，替换 skill→case 术语、配色、API 调用。LeanStartup 已有 `pages/history/detail.vue`（案例详情页，保留不动），需要替换的是 `pages/history/index.vue`（当前是占位符）。
...
**Files:**
- Replace: `src/pages/history/index.vue`

从 LeanSkill 的 `pages/history/index.vue` 适配。

**关键适配：**
- `useSkillStore` → `useCaseStore`
- `Ranking` type → `HistoryItem` type
- `ranking._id` → `item.date`（用 date 作为 key）
- `ranking.skills` → `item.case_ids`
- `store.getSkillById(id)` → `store.getCaseById(id)`
- 颜色替换：`#0F172A`→`#1A1A2E`, `#F97316`→`#E94560`, `#E2E8F0`→`#E8E6E1`, `#F1F5F9`→`#F5F5F3`
- 路由跳转：`/pages/history/list?date=xxx` → `/pages/history/detail?date=xxx`
- 路由标题改为 `精益副业案例库 · 往期榜单`
- 不使用 `wd-icon` 和 `wd-button`，用纯文本替代
...
- [ ] **Step 1:** 替换 `src/pages/history/index.vue`
- [ ] **Step 2:** Commit

```bash
git add src/pages/history/index.vue && git commit -m "feat: rewrite history list page adapted from LeanSkill"
```
